...
def classify_category(order_text: str) -> str:
    ...
    if any(w in t for w in ["ally", "treaty", "envoy", "negotiate", "sanction", "peace"]):
        return "diplomacy"
    if any(w in t for w in ["temple", "priest", "festival", "edict", "faith", "religion", "church","God"]):
        return "religion"
    return "generic"
...
def apply_small_variance(effect: Dict[str, Any], variance: float = LOWER_VARIANCE) -> Dict[str, Any]:
